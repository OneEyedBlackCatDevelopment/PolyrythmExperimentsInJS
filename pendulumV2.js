...
function updatePendulums() {
	...
	//	}
		...
		if( pendulum.theta >=  ( theta0 - 0.01 ) || pendulum.theta <= (-theta0+0.01) ) {
			drawColor = '#FFFFFF';
			if( pendulum.allreadyHit == false ) {
				pendulum.allreadyHit = true;
				
				pendulum.panner.pan.value = Math.sign(pendulum.theta);
				playSound( pendulum.audioPlayerId);
			}
		} else {
			pendulum.allreadyHit = false;
...
function createPendulums() {
	 ...
	 // Create 10 pendulums with individual length controls
            for (let i = 1; i <= 9; i++) {
                const pendulumLength = 100 * Math.sqrt(i);
				...
				// for left right panning 
				let audioCtx;
				audioCtx = new AudioContext();
				let source = new MediaElementAudioSourceNode(audioCtx, { mediaElement: audioPlayer, });
				let panNode = new StereoPannerNode(audioCtx);
				panNode.pan.value = -1; //panControl.value;
